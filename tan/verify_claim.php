...
<head>
    ...
    <style>
        ...
        .admin-header {
            ...
            border-radius: 0 0 16px 16px; display: flex;
...
<?php if (!$is_admin): ?>
    <div class="admin-login-wrapper">
        <div class="glass-card text-center" style="max-width: 400px; width: 100%;">
            ...
            <p class="text-muted">Enter credentials to manage UTM claims</p>
            <?php if (!empty($error)): ?>
                <div class="alert alert-danger alert-custom alert-custom-danger py-2 px-3 mb-3"><?php echo htmlspecialchars($error); ?></div>
            <?php endif; ?>
            <form method="POST">
            ...
            <div class="mt-4 pt-3 border-top">
                <p class="text-muted m-0">Default Password: <strong>admin123</strong></p>
                <a href="../index.php" style="font-size: 13px; color: var(--text-muted); text-decoration: none;">🏠 Back to Portal</a>
            </div>
        </div>
    </div>
    <?php exit(); ?>
<?php endif; ?>
    ...
    <div class="app-container" style="max-width: 1100px; margin-top: 0; padding-top: 0;">
        ...
        <?php if (!empty($message)): ?>
            <div class="alert alert-success alert-custom alert-custom-success mb-4"><?php echo htmlspecialchars($message); ?></div>
        <?php endif; ?>

        <div class="row g-3 mb-4">
            <div class="col-md-4"><div class="p-3 border rounded bg-white text-center shadow-sm">⏳ <h3 class="m-0 text-warning fw-bold mt-1"><?php echo $stats_pending; ?></h3><small class="text-muted fw-bold">Pending Claims</small></div></div>
            <div class="col-md-4"><div class="p-3 border rounded bg-white text-center shadow-sm">✅ <h3 class="m-0 text-success fw-bold mt-1"><?php echo $stats_verified; ?></h3><small class="text-muted fw-bold">Verified Claims</small></div></div>
            <div class="col-md-4"><div class="p-3 border rounded bg-white text-center shadow-sm">🎉 <h3 class="m-0 text-info fw-bold mt-1"><?php echo $stats_returned; ?></h3><small class="text-muted fw-bold">Returned Items</small></div></div>
        </div>

        <div class="glass-card mb-5">
            <h3>📋 Pending Claims Review (<?php echo $pending_result->num_rows; ?>)</h3>
            <?php if ($pending_result->num_rows > 0): ?>
                        ...
                        <div class="row g-3 align-items-center">
                            <div class="col-md-9">
                                <div class="row">
                                    <div class="col-md-6 border-end">
                                        <p class="m-0 text-muted" style="font-size: 11px;">CLAIMANT DETAILS</p>
                                        <h4 class="m-0 fw-bold"><?php echo htmlspecialchars($row['owner_name']); ?></h4>
                                        <p class="m-0 text-muted"><?php echo htmlspecialchars($row['owner_email']); ?> | <?php echo htmlspecialchars($row['phone']); ?></p>
                                        <p class="m-0 mt-3 text-muted" style="font-size: 11px;">MATCH PAIR</p>
                                        <p class="m-0"><strong>Lost:</strong> <?php echo htmlspecialchars($row['lost_item']); ?></p>
                                        <p class="m-0"><strong>Found:</strong> <?php echo htmlspecialchars($row['found_item']); ?></p>
                                    </div>
                                    <div class="col-md-6 ps-md-4">
                                        <p class="m-0 text-muted" style="font-size: 11px;">OWNERSHIP PROOF DESCRIPTION</p>
                                        <p class="m-0 text-dark"><?php echo htmlspecialchars($row['proof_description']); ?></p>
                                        <?php if ($row['proof_url']): ?>
                                            <p class="m-0 mt-3"><a href="../<?php echo htmlspecialchars($row['proof_url']); ?>" target="_blank" class="btn-custom btn-custom-outline py-1 px-3 mt-1" style="font-size: 11px;">📎 View Uploaded Document</a></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 border-start text-center">
                                <?php if ($row['photo_url']): ?>
                                    <img src="../<?php echo htmlspecialchars($row['photo_url']); ?>" alt="Item" class="img-fluid rounded mb-3 shadow-sm" style="max-height: 100px; object-fit: cover;">
                                <?php endif; ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <div class="text-center py-5 text-muted bg-light border rounded"><p class="m-0">No pending claims for review. All caught up!</p></div>
            <?php endif; ?>
        </div>

        <div class="glass-card">
            <h3>📊 Historical Claims Log</h3>
            <div class="table-responsive">
                <table class="custom-table">
                    <thead><tr><th>Claim ID</th><th>Claimant</th><th>Lost Item</th><th>Found Item</th><th>Status</th><th>Date Filed</th></tr></thead>
                    <tbody>
